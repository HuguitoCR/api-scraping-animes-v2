...
const animeInfo = async (anime_id, res) => {
	const $ = await getHtml(`https://www.animefenix.tv/${anime_id}`);
	const info = [];
	const episodes = [];
	...
	$('ul.anime-page__episode-list li').each(function () {
		...
		const episode = {
			...
			episode_id: $this.find('a').attr('href').split('https://www.animefenix.tv/ver/')[1],
		};
